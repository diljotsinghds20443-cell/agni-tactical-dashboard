...
// Enhanced Hold Control with Touch Cancellation & Ghost Event Prevention
function attachHoldControl(buttonId, dir) {
    ...
    btn.addEventListener('touchstart', (e) => {
        e.preventDefault(); // Prevents zoom, scroll, and ghost mouse events
        isTouching = true;
    ...
    // Safety fallback if finger slips off or phone interrupts touch
    ...
    // --- Mouse Controls (Laptop/PC Only) ---
    btn.addEventListener('mousedown', (e) => {
        if (isTouching) return; // Ignore fake mouse clicks generated by phone
        e.preventDefault();
        setMotorDirection(dir);
    });
    ...
    btn.addEventListener('mouseleave', () => {
        if (!isTouching && STATE.motorDirection === dir) {
            setMotorDirection('STOP');
        }
    });
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // D-Pad Hold-to-Move Button Bindings
    attachHoldControl('btnUp', 'FORWARD');
    attachHoldControl('btnDown', 'BACKWARD');
    attachHoldControl('btnLeft', 'LEFT');
    attachHoldControl('btnRight', 'RIGHT');

    // Emergency Stop Button
    const btnStop = document.getElementById('btnStop');
    if (btnStop) {
        btnStop.onclick = () => setMotorDirection('STOP');
    }
